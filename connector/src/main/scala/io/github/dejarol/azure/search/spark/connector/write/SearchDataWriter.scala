...
import io.github.dejarol.azure.search.spark.connector.write.decoding.SearchDocumentDecoder
...
class SearchDataWriter(
  ...
  override def close(): Unit = log.debug(s"Closing writing task $taskId on partition $partitionId")
